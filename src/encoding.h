...
#ifndef S3_ENCODING_H
#define S3_ENCODING_H
...
namespace s3
{
  ...
  public:
    ...
    // TODO: deprecate this?
    inline static std::string encode(const uint8_t *input, size_t size, encoding_type type)
    {
      if (type == E_BASE64)
        return crypto::base64::encode(input, size);
    ...
    inline static std::string encode(const std::string &input, encoding_type type)
    {
      return encode(reinterpret_cast<const uint8_t *>(input.c_str()), input.size() + 1, type);
    }

    inline static std::string encode(const std::vector<uint8_t> &input, encoding_type type)
    {
      return encode(&input[0], input.size(), type);
    }

    inline static void decode(const std::string &input, encoding_type type, std::vector<uint8_t> *output)
    {
      if (type == E_BASE64)
        crypto::base64::decode(input, output);
      else if (type == E_HEX)
    ...
    inline static std::string encode_hash(const uint8_t *input, size_t size, encoding_type type)
    {
      uint8_t hash[hash_type::HASH_LEN];

      hash_type::compute(input, size, hash);

      return encode(hash, hash_type::HASH_LEN, type);
    }

    template <class hash_type>
    inline static std::string encode_hash(int fd, encoding_type type)
    {
      uint8_t hash[hash_type::HASH_LEN];
