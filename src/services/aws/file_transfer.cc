...
int file_transfer::upload_multi(const std::string &url, size_t size,
  ...
  if (r) {
    threads::pool::call(threads::PR_REQ_0,
                        bind(&file_transfer::upload_multi_cancel, this,
                             std::placeholders::_1, url, upload_id));
  ...
  return threads::pool::call(threads::PR_REQ_0,
                             bind(&file_transfer::upload_multi_complete, this,
                                  std::placeholders::_1, url, upload_id,
                                  complete_upload, returned_etag));
}
...
int file_transfer::upload_part(const base::request::ptr &req,
  ...
  range->etag =
      crypto::hash::compute<crypto::md5, crypto::hex_with_quotes>(*buffer);

  req->init(base::HTTP_PUT);

  // part numbers are 1-based
  req->set_url(url + "?partNumber=" + std::to_string(range->id + 1) +
               "&uploadId=" + upload_id);
  ...
  if (req->get_response_header("ETag") != range->etag) {
    S3_LOG(LOG_WARNING, "file_transfer::upload_part",
           "md5 mismatch. expected %s, got %s.\n", range->etag.c_str(),
           req->get_response_header("ETag").c_str());
...
int file_transfer::upload_multi_init(const base::request::ptr &req,
  ...
  if (!doc) {
    S3_LOG(LOG_WARNING, "file_transfer::upload_multi_init",
...
int file_transfer::upload_multi_cancel(const base::request::ptr &req,
                                       const std::string &url,
                                       const std::string &upload_id) {
  S3_LOG(LOG_WARNING, "file_transfer::upload_multi_cancel",
...
int file_transfer::upload_multi_complete(const base::request::ptr &req,
                                         const std::string &url,
                                         const std::string &upload_id,
  ...
  if (req->get_response_code() != base::HTTP_SC_OK) {
    S3_LOG(LOG_WARNING, "file_transfer::upload_multi_complete",
