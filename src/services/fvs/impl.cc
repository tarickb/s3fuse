...
impl::impl()
{
  ...
  crypto::private_file::open(base::config::get_fvs_secret_file(), &f);
  getline(f, line);

  std::istringstream line_stream(line);
  std::vector<std::string> fields{std::istream_iterator<std::string>(line_stream),
  ...
  _endpoint = base::config::get_fvs_use_ssl() ? "https://" : "http://";
  _endpoint += base::config::get_fvs_service_endpoint();
...
void impl::sign(base::request *req)
{
  ...
  for (base::header_map::const_iterator itor = headers.begin(); itor != headers.end(); ++itor)
  ...
  req->set_header("Authorization", std::string("IIJGIO ") + _key + ":" +
      crypto::encoder::encode<crypto::base64>(mac, crypto::hmac_sha1::MAC_LEN));
}
...
{
  return std::shared_ptr<file_transfer>(new fvs::file_transfer());
}
} } }
