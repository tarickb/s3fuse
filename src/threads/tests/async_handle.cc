...
void set_value(int i, int *r) { *r = i; }
} // namespace
...
TEST(wait_async_handle, signal_after_wait) {
  ...
  std::unique_ptr<std::thread> t(
      new std::thread(std::bind(delay_signal_handle, h, 321)));
...
TEST(callback_async_handle, callback) {
  int r = 0;
  callback_async_handle::ptr h(
      new callback_async_handle(std::bind(set_value, 444, &r)));
