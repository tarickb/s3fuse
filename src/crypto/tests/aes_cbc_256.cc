...
TEST(aes_cbc_256, invalid_iv_len)
{
  ...
  EXPECT_THROW((cipher::encrypt<aes_cbc_256_with_pkcs, hex>(sk, "foo")), std::runtime_error);
}
...
TEST(aes_cbc_256, decrypt_on_invalid_size)
{
  ...
  EXPECT_THROW((cipher::decrypt<aes_cbc_256_with_pkcs, hex>(sk, "aa")), std::runtime_error);
}
...
TEST(aes_cbc_256, decode_non_null_terminated_string)
{
  ...
  EXPECT_THROW(cipher::decrypt<aes_cbc_256_with_pkcs>(sk, &out[0], out.size()), std::runtime_error);
}
