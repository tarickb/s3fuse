...
namespace s3
{
  ...
  public:
    ...
    int commit()
    {
      return thread_pool::call(PR_REQ_0, boost::bind(&object::commit, shared_from_this(), _1));
    }

    int remove()
    {
      return thread_pool::call(PR_REQ_0, boost::bind(&object::remove, shared_from_this(), _1));
    }

    int rename(const std::string &to)
    {
      return thread_pool::call(PR_REQ_0, boost::bind(&object::rename, shared_from_this(), _1, to));
    }
